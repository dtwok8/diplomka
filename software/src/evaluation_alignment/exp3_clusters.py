...
def run():
	alels_dictionary = create_alels_dictionary()
	...
	for genotype_aligment_file in aligments_sam_files:
		...
		for cluster in clusters:
			...
			for item in cluster:
				if(alels_statistics[item]['normalize_coverage'] > config.CUT_COVERAGE_ALELS):
					stay = True
					break
		...
		new_align_file = make_new_reference_and_align.align("/home/kate/Dokumenty/FAV/Diplomka/software/data/temp", new_reference, basic_name, "_exp3_step2")
